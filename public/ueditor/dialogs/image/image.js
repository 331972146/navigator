    ...
    UploadImage.prototype = {
        ...
        initUploader: function () {
            ...
            uploader = _this.uploader = WebUploader.create({
                ...
                accept: {  // by AlexGZhou, 设置断点, 查看X-Requested-With是否设置
                    title: 'Images',
                ...
                server: actionUrl,  // by AlexGZhou, 设置断点, 查看actionUrl是什麽
            ...
            function addFile(file) {
                ...
                if (file.getStatus() === 'invalid') {  // 下面的uploader.on('error')事件把'Q_TYPE_DENIED'文件类型错误也设置为'invalid', 因此, /* 检查文件格式 */的代码不会执行
                    showError(file.statusText);
                    // console.log(uploader.getFiles('invalid'));
                }
                else {
                    ...
                    /* 检查文件格式 */  // 这里的代码不会执行, 因为类型'Q_TYPE_DENIED'错误的文件也被设置为'invalid', 被上面的if条件拦截
                    if (!file.ext || acceptExtensions.indexOf(file.ext.toLowerCase()) == -1) {
                        showError('not_allow_type');
                        // console.log('ext');
                        uploader.removeFile(file);  // 标记为cancelled
                        // uploader.removeFile(file, true);  // 直接从列队删除, 而不是标记为cancelled
                    }
                }
                    ...
                    switch (index) {
                        case 0:
                            if (file.getStatus() === 'invalid') {
                                ...
                                // // console.log(files);
                                // while (--len >= 0) {
                                //     console.log(uploader.getFiles()[len].getStatus() + ', ' + len);
                                // }
            ...
            //     // console.log(file);
            //     // return false;
            //     var files = uploader.getFiles('inited', 'queued', 'progress', 'error', 'interrupt', 'complete');
            ...
            uploader.on('fileQueued', function (file) {
                ...
                uploader.md5File(file)
                    ...
                    .progress(function(percentage) {
                        ...
                        // percentages[ file.id ][ 1 ] = percentage;
                        // updateTotalProgress();
                        $info.text(info + ', ' + file.statusText + Math.round(percentage * 100) + "%");
                    })
                    // 完成
                    .then(function(val) {
                        // console.log('md5 result:', val);
            ...
            uploader.on('fileDequeued', function (file) {
                ...
                if ( !fileCount ) {
                    setState('pedding');
                    $("[id^='WU_FILE_']").remove(); // 这里需要删除所有file.id('WU_FILE_'开头)的元素, 其实是'invalid'的file, 否则, 当最后一个有效的文件被删除后, 面板就会初始化setState('pedding'), 这时那些无效的元素还是存在的, 一旦再次加入文件, 就会重新显示出来
                }
            ...
            uploader.on('uploadBeforeSend', function (obj, data, header) {
                ...
                data.token = sessionStorage.getItem('token');  // 这里要加入token, 以便于服务器端解析获取用户_id
            ...
            uploader.on('uploadAccept', function (obj, ret) {
                ...
                try {
                    ...
                    if (json.state == 'SUCCESS') {
                        ...
                        obj.file.setStatus('error', json.state);
                        return false;
                    }
                } 
                catch (e) {
            ...
            uploader.on('error', function (code, file, max) {
                // console.log(code);
                // console.log(max);
                // console.log(file);
                if (code == 'Q_TYPE_DENIED') {
                    file.setStatus('invalid', 'not_allow_type');
                    addFile(file);
                }
                if (code == 'F_EXCEED_SIZE') {
                    // console.log(max.getStatus());
                    addFile(max);
                }
        ...
        getQueueCount: function () {
            ...
            for (i = 0; file = files[i++]; ) {
                ...
                // console.log(status);
                if (status == 'inited' || status == 'queued' || status == 'error' || status == 'interrupt') readyFile++;
            }
            return readyFile;
        },
        destroy: function () {
        ...
        getImageData: function () {
            ...
            if(!_this.listEnd && !this.isLoadingData) {
                ...
                ajax.request(url, {
                    ...
                    'data': utils.extend({
                            token: sessionStorage.getItem('token'),  // 这里(req.query.token)要加入token, 以便于服务器端解析获取用户_id
